    ...
    public class GoCameraSystem : GoSystemsBehaviour
    {
        ...
        private void PositionCameraBehaviour()
        {
            if (Gs.IsCameraInput)
            {
                if (!lockcode)
                {
                    switch (InputCameraSystem.MoveAxis)
                    {


                        case input3.ComputerAndConsul:
                            x += Input.GetAxis(mosuex) * CameraSensitivity;
                            y -= Input.GetAxis(mosuey) * CameraSensitivity;
                            MovePosition(x, y);
                            break;
                        case input3.MobileJoystick:
                            x 
                                += InputCameraSystem.TouchSpace.newCameraAxis.x * InputCameraSystem.panSpeed;
